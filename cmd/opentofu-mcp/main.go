...
import (
	...
	"github.com/Nathan-E-White/opentofu-codex-plugin/internal/mcpapp"
...
func main() {
	...
	server := mcp.NewServer(&mcp.Implementation{Name: "opentofu", Version: "0.3.0"}, &mcp.ServerOptions{
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Meta:        mcpapp.ToolMeta("Inspecting OpenTofu context", "OpenTofu preflight ready"),
		...
		return &mcp.CallToolResult{Meta: mcpapp.ResultMeta("opentofu_preflight")}, out, err
	})
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Meta:        mcpapp.ToolMeta("Running OpenTofu policy gates", "OpenTofu policy evidence ready"),
		...
		return &mcp.CallToolResult{Meta: mcpapp.ResultMeta("opentofu_policy_check")}, out, err
	})
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Meta:        mcpapp.ToolMeta("Planning OpenTofu changes", "OpenTofu plan ready"),
		...
		return &mcp.CallToolResult{Meta: mcpapp.ResultMeta("opentofu_plan")}, out, err
	})
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Meta:        mcpapp.ToolMeta("Applying confirmed OpenTofu plan", "OpenTofu apply finished"),
		...
		return &mcp.CallToolResult{Meta: mcpapp.ResultMeta("opentofu_execute_plan")}, out, err
	})
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Meta:        mcpapp.ToolMeta("Loading OpenTofu evidence", "OpenTofu evidence loaded"),
		...
		return &mcp.CallToolResult{Meta: mcpapp.ResultMeta("opentofu_get_evidence")}, out, err
	})
